...
def _strip_accents(text: str) -> str:
    """Best-effort remove Vietnamese accents for robust keyword matching."""
...
def parse_intent_llm(user_input, has_file=False):
    ...
    1. SEARCH_NAME: Người dùng ĐƯA RA TÊN BÀI HÁT CỤ THỂ để nghe (VD: "Mở bài Nơi này có anh", "Tìm bài Chạy Ngay Đi của Sơn Tùng"). 
       *Cấm dùng nếu chỉ nhắc đến tên ca sĩ mà không có tên bài. Cấm dùng nếu câu lệnh chứa các thuật ngữ sản xuất âm nhạc/nhạc lý (compressor, bass, beat, mix, master, viết nhạc).*
    ...
    2. SEARCH_LYRIC: Tìm bài hát thông qua MỘT ĐOẠN LỜI (VD: "Bài gì có câu mang tiền về cho mẹ").
       *Ưu tiên action này nếu có từ khóa "lời", "lyrics", "câu". TUYỆT ĐỐI KHÔNG dùng nếu câu hỏi mang tính chất hướng dẫn/làm sao để (VD: 'làm sao để viết một đoạn...'), lúc này phải dùng MUSIC_KNOWLEDGE.*
    ...
    6. RECOMMEND_GENRE: Gợi ý theo thể loại (Rap, Pop, Ballad, Indie...).
        *Đặc biệt: Gộp chung các từ lóng mô tả không khí club (xập xình, quẩy, remix, vinahouse) vào Thể Loại này, KHÔNG đưa vào Advanced Search. Cấm dùng action này nếu người dùng đang hỏi về lịch sử, định nghĩa hoặc sự phát triển của thể loại đó (VD: 'V-Pop là gì', 'phát triển từ khi nào').*
    ...
    15. MUSIC_KNOWLEDGE: Trả lời các câu hỏi về kiến thức âm nhạc, lý thuyết, nhạc lý, kỹ thuật phòng thu, hoặc tiểu sử nghệ sĩ. "
        "DẤU HIỆU: Câu hỏi chứa từ nghi vấn (là gì, thế nào, bao nhiêu, tại sao, ai là, hướng dẫn, cách để). HOẶC câu hỏi yêu cầu liệt kê thông tin, kiến thức (VD: 'Những nghệ sĩ...', 'Các dòng nhạc...') mà KHÔNG yêu cầu mở/phát nhạc."
        "Ví dụ: 'Sơn Tùng sinh năm bao nhiêu?', 'Synthwave là gì?', 'Cách mix vocal'."
...
def _heuristic_intent(user_input: str, *, has_file: bool, thought: str) -> dict:
    ...
    text = normalize(raw)
    text_unaccent = _strip_accents(raw)
    method = 'Rule'

    # --- MUSIC_KNOWLEDGE ưu tiên: chặn rơi nhầm vào SEARCH_NAME/RECOMMEND_* ---
    knowledge_tokens = [
        ...
        "tiểu sử", "tieu su",
        # mở rộng nhẹ để cover template MUSIC_KNOWLEDGE
    ...
    has_knowledge = any(k in text_unaccent for k in knowledge_tokens)
    # pattern liệt kê/tiểu sử/lịch sử
    is_list_like_knowledge = bool(
        re.search(r"^(nhung|cac)\s+", text_unaccent)
        and re.search(r"\b(nghe\s*si|nhac\s*si|nhac\s*cu|dong\s+nhac)\b", text_unaccent)
        and not re.search(r"\b(bai\s+hat|playlist|list\s+nhac)\b", text_unaccent)
    )
    # đừng override khi có lệnh nghe/tìm bài rõ ràng
    is_explicit_play = bool(re.search(r"\b(mo|bat|nghe|phat|tim\s+bai|ten\s+bai|playlist|list\s+nhac)\b", text_unaccent))
    if re.search(r"\btim\s+hieu\b", text_unaccent):
        is_explicit_play = False

    if (has_knowledge or is_list_like_knowledge) and (not is_explicit_play) and (not has_file):
        return {
    ...
    # --- RECOMMEND_ATTRIBUTES (Bắt tín hiệu Tempo & Energy) ---
    attribute_keywords = [
        # Tempo
        "chậm", "cham", "slow", "vừa", "vua", "bình thường", "binh thuong", 
        "nhanh", "fast", "dồn dập", "don dap", "speed up", "tempo", "bpm", "nhịp", "nhip",
        # Energy
        "thấp", "thap", "yếu", "yeu", "nhẹ", "nhe", "êm dịu", "em diu", "mỏng manh", "mong manh", 
        "nhẹ tênh", "nhe tenh", "cao", "mạnh", "manh", "uy lực", "uy luc", "căng", "cang", 
        "dày", "day", "năng lượng", "nang luong"
    ]
    
    # Kiểm tra xem có keyword attribute nào kết hợp với các từ ngữ chuyên ngành không
    found_attr = [k for k in attribute_keywords if k in text]
    
    if found_attr:
        # Loại trừ trường hợp trùng lặp với Mood (VD: "nhạc buồn", "chill" thường đi chung)
        # Chỉ chốt RECOMMEND_ATTRIBUTES nếu đây là focus chính của câu hỏi
        is_attribute_focus = any(k in text for k in ["tempo", "bpm", "nhịp", "nhip", "năng lượng", "nang luong"]) or len(found_attr) >= 2
        
        # Nếu có chữ "nhanh", "chậm", "căng", "dày" đứng riêng rẽ mà không bị đè bởi Mood/Genre rõ ràng
        if is_attribute_focus or not (any(m in text for m in mood_map.values()) or any(g in text for g in genre_map.values())):
            params = _empty_params()
            # Gom các từ tìm thấy vào params 'attributes'
            params['attributes'] = ", ".join(found_attr)
            
            # Gỡ gạc luôn các biến số có thể xuất hiện
            if "tempo" in text or "bpm" in text:
                 import re
                 num_match = re.search(r'\d+', text)
                 if num_match:
                     params['attributes'] += f", {num_match.group()}"

            return {
                "action": "RECOMMEND_ATTRIBUTES",
                "params": params,
                "thought": thought,
                'method': method,
            }
