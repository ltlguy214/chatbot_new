...
def call_gemini_engine(prompt, *, module: str | None = None):
    ...
    # Quota cooldown: if we recently hit 429, don't keep retrying every rerun.
    try:
        until = float(st.session_state.get('gemini_quota_cooldown_until') or 0.0)
    except Exception:
        until = 0.0
    try:
        now = time.time()
    ...
    if until and now and now < until:
        ...
        st.session_state['gemini_last_error'] = f'quota-cooldown:{remaining}s'
        return None
            ...
            for candidate in model_candidates:
                try:
                    ...
                    if _looks_like_quota_error(ex):
                        try:
                            st.session_state['gemini_quota_cooldown_until'] = time.time() + 10 * 60
                        except Exception:
                            pass
                        break
    except Exception as ex:
